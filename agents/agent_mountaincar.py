...
class DDPG_Agent():
    ...
    def configure(self, gamma=0.9, tau=0.1, tau_critic=0.5, tau_actor=0.1, buffer_size=100000, batch_size=128, exploration_mu=0,
                  exploration_theta=0.15, exploration_sigma=0.2, learn_during_episode=True, throttle=1):
    ...
    def step(self, action, reward, next_state, done):
        ...
        if len(self.memory) > self.batch_size:
            if self.learn_during_episode or done:
    ...
    def act(self, state, p=0):
        ...
        action = action * self.throttle + (p * self.noise.sample())
...
class Critic:
    ...
    def build_model(self):
        ...
        net_states = layers.Dense(units=300, activation='elu', kernel_initializer=kernel_initializer)(net_states)

        # Add hidden layer(s) for action pathway
        net_actions = layers.Dense(units=400, activation='elu', kernel_initializer=kernel_initializer)(actions)
        net_actions = layers.Dense(units=300, activation='elu', kernel_initializer=kernel_initializer)(net_actions)
